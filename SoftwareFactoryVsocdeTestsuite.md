...
- offline docs MCP
- GitHub ops MCP
- context7 wrapper/service if included
...
3. Implement automated fresh-install tests that validate bootstrap into a blank host repo.
4. Implement automated runtime smoke tests that validate env generation, compose startup, health endpoints, `/target` visibility, and teardown.
5. Implement automated dual-host isolation tests proving two separate host repos can run concurrently without collisions or shared state contamination.
6. Implement automated upgrade tests proving version lock handling and host override preservation.
7. Implement failure-mode tests for missing prerequisites, bad config, malformed overrides, and occupied ports.
8. Implement documentation/testing smoke checks so that install and upgrade docs match the actual commands and paths.
9. Wire all mandatory tests into CI release gates.
10. Leave the repository in a state where the test suite can be executed repeatedly and deterministically.
