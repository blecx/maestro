"""MaestroOrchestrator — top-level MAESTRO agent pipeline.

Wires together:
  RouterAgent → CoderAgent → (PR creation) → (memory lesson)

One call to ``MaestroOrchestrator.run_issue()`` does everything needed to
implement a GitHub issue end-to-end.

Typical usage (programmatic):
    orq = MaestroOrchestrator(server_urls={...})
    result = await orq.run_issue(42, "blecx/AI-Agent-Framework", "Fix bug", "Body...")
    print(result.pr_url)
...
def _load_server_urls_from_env() -> dict[str, str]:
    """Build server URL dict from MAESTRO_*_URL env vars, falling back to defaults."""
    mapping = {
...
class OrchestratorResult:
    """End-to-end result of running MAESTRO on one issue."""
...
class MaestroOrchestrator:
    ...
    def __init__(
        ...
        Args:
            server_urls:    Dict of {server_name: url}.  Falls back to
                            MAESTRO_*_URL env vars or default localhost ports.
    ...
    async def run_issue(
        ...
        """Execute the full MAESTRO pipeline for one GitHub issue.
    ...
    async def _create_pr(
        ...
        branch = f"maestro/issue-{issue_number}"
        title = f"feat: {issue_title} (#{issue_number})" if issue_title else f"feat: issue #{issue_number}"
        body = (
            f"Implements #{issue_number}\n\n"
            f"Generated by MAESTRO (run `{run_id}`).\n\n"
            f"**Files changed:**\n" + "\n".join(f"- `{f}`" for f in files_changed)
        )
        try:
            result: Any = await mcp.call_tool("create_pr", {
                "repo": repo,
                "title": title,
                "body": body,
                "head": branch,
    ...
    async def _store_lesson(
        ...
        try:
            await mcp.call_tool("memory_store_lesson", {
                "issue_number": issue_number,
                "repo": repo,
                "complexity_score": decision.complexity_score,
