...
class MCPBootloader:
    ...
    def _hydrate_state(self):
        ...
        snapshot_dir = self.workspace_root / ".tmp" / "softwareFactoryVscode" / "snapshots"
        ...
        for service, dest_path, snap_path in dbs:
            ...
            try:
                ...
                if res.returncode == 0:
                    ...
                    tmp_bak = f"/factory/tmp/{snap_path.name}.bak"
        ...
        try:
            subprocess.run(
                ...
                + [
                    ...
                    "python",
                    "-c",
                    "import pathlib, sqlite3; pathlib.Path('/factory/tmp').mkdir(parents=True, exist_ok=True); con=sqlite3.connect('/data/memory.db'); bck=sqlite3.connect('/factory/tmp/memory.db.bak'); con.backup(bck); bck.close(); con.close()",
                ],
                cwd=self.workspace_root,
                check=True,
            ...
            subprocess.run(
                ...
                + [
                    "cp",
                    "mcp-memory:/factory/tmp/memory.db.bak",
        ...
        try:
            subprocess.run(
                ...
                + [
                    ...
                    "python",
                    "-c",
                    "import pathlib, sqlite3; pathlib.Path('/factory/tmp').mkdir(parents=True, exist_ok=True); con=sqlite3.connect('/data/agent_bus.db'); bck=sqlite3.connect('/factory/tmp/agent_bus.db.bak'); con.backup(bck); bck.close(); con.close()",
                ],
                cwd=self.workspace_root,
                check=True,
            ...
            subprocess.run(
                ...
                + [
                    "cp",
                    "mcp-agent-bus:/factory/tmp/agent_bus.db.bak",
